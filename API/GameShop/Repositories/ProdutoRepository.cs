...
public class ProdutoRepository(NpgsqlDataSource ds)
{
    ...
    public bool Delete(int id)
    {
        ...
        using var cmdCheck = new NpgsqlCommand(
            "SELECT COUNT(*) FROM item_venda WHERE id_produto = @id", conn, tx);
    ...
    {
        using var conn = ds.OpenConnection();
        using var cmd = new NpgsqlCommand("SELECT COALESCE(MAX(id), 0) FROM produto", conn);
        var maxId = (long)(int)cmd.ExecuteScalar()!;
        return $"PROD-{(maxId + 1):D4}";
    }
